/* global artifacts */
/* global contract */
/* global web3 */
/* global assert */
const AdderV2 = artifacts.require('RedPacket');
// const SolRsaVerify = artifacts.require('../contracts/SolRsaVerify.sol')

contract('RedPacket', (accounts) => {
  beforeEach(async () => {
    v = await SolRsaVerify.new()
  })

  /*
        > openssl genrsa -out private.pem 1024

        > cat private.pem
        -----BEGIN RSA PRIVATE KEY-----
        
        -----END RSA PRIVATE KEY-----

        > openssl rsa -in private.pem -outform der -pubout -out public.pem

        # addd -n to echo to remove final carriage return
        > echo -n "hello world" | openssl dgst -sha256 -sign private.pem -out | xxd -p | tr -d \\n

        

        # extract the public n from the public key
        > openssl asn1parse -inform TXT -i -in public.pem -strparse 18
            0:d=0  hl=3 l= 137 cons: SEQUENCE
            3:d=1  hl=3 l= 129 prim:  INTEGER           :
          135:d=1  hl=2 l=   3 prim:  INTEGER           :010001
        
        # get the `e`=010001 and `n` DF..0D
    */

  it('openssln generation test', async () => {
    const e =
      ''
    const Msg = web3.utils.asciiToHex('hello world')

    console.log(Msg)
    // const Msg2 = web3.utils.asciiToHex("cf6c9723526e1bfa09f32eb6be0f59e06320f5ed443a08e63dde7d3d9f4af5fa2e8711bb0eee5c999087954dcab683b995c42987")
    // console.log(Msg2);
    const S =
      ''
    const nn =
      ''
    console.log(
      (
        await v.pkcs1Sha256VerifyRaw(Msg, '0x' + S, '0x' + e, '0x' + nn)
      ).toNumber()
    )
    assert.equal(
      (
        await v.pkcs1Sha256VerifyRaw(Msg, '0x' + S, '0x' + e, '0x' + nn)
      ).toNumber(),
      0
    )
  })
})
